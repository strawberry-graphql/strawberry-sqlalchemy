...
def created_default_secondary_table_data(session, employee_model, department_model):
    department1 = department_model(id=10, name="Department Test 1")
    department2 = department_model(id=3, name="Department Test 2")
    e1 = employee_model(id=1, name="John", role="Developer")
    e2 = employee_model(id=5, name="Bill", role="Doctor")
    e3 = employee_model(id=4, name="Maria", role="Teacher")
    ...
    return e1, e2, e3, department1, department2
...
async def test_query_with_secondary_table_with_values_list_without_list_connection(
    ...
    async with async_sessionmaker(expire_on_commit=False) as session:
        created_default_secondary_table_data(
            session=session,
...
async def test_query_with_secondary_tables_with_more_than_2_colluns_values_list(
    secondary_tables_with_more_secondary_tables, base, async_engine, async_sessionmaker
):
    async with async_engine.begin() as conn:
        await conn.run_sync(base.metadata.create_all)
    ...
    async with async_sessionmaker(expire_on_commit=False) as session:
        ...
        e1, e2, e3, _, _ = created_default_secondary_table_data(
            session=session,
            employee_model=EmployeeModel,
            department_model=DepartmentModel,
        )
...
async def test_query_with_secondary_table(
    secondary_tables_with_use_list_false, base, async_engine, async_sessionmaker
):
    async with async_engine.begin() as conn:
        await conn.run_sync(base.metadata.create_all)
    mapper = StrawberrySQLAlchemyMapper()
...
async def test_query_with_secondary_table_with_values_list_and_normal_relationship(
    secondary_tables_with_normal_relationship, base, async_engine, async_sessionmaker
):
    async with async_engine.begin() as conn:
        await conn.run_sync(base.metadata.create_all)
